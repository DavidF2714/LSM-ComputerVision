...
def get_hand_bbox(landmarks, image_width, image_height, margin=15):
    x_coords = [lm.x for lm in landmarks.landmark]
    y_coords = [lm.y for lm in landmarks.landmark]
    
    x_min = max(int(min(x_coords) * image_width) - margin, 0)
    x_max = min(int(max(x_coords) * image_width) + margin, image_width)
    y_min = max(int(min(y_coords) * image_height) - margin, 0)
    y_max = min(int(max(y_coords) * image_height) + margin, image_height)
        ...
        for file in files:
            if file.endswith(('.jpg', '.png')):
                ...
                with open(CSV_FILE, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([file, hands_found, left_landmarks, right_landmarks, cropped_hand])
